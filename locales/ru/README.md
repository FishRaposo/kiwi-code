...
## Что Kiwi Code может сделать для ВАС?
...
Kiwi Code адаптируется к вашему стилю работы, а не наоборот:
...
Подробнее: [Использование режимов](https://docs.kiwicode.dev/basic-usage/using-modes) • [Пользовательские режимы](https://docs.kiwicode.dev/advanced-usage/custom-modes)
...
<p align="center">
<a href="https://docs.kiwicode.dev/tutorial-videos">Больше коротких обучающих видео и видео о функциях...</a>
</p>

## Ресурсы

- **[Документация](https://docs.kiwicode.dev):** Официальное руководство по установке, настройке и освоению Kiwi Code.
...
Существует несколько способов запустить расширение Kiwi Code:

### Режим разработки (F5)

Для активной разработки используйте встроенную отладку VSCode:

Нажмите `F5` (или перейдите в меню **Выполнить** → **Начать отладку**) в VSCode. Это откроет новое окно VSCode с запущенным расширением Kiwi Code.
...
1.  Сначала соберите пакет VSIX:
    ...
    pnpm vsix
    ```
2.  Файл `.vsix` будет сгенерирован в каталоге `bin/` (например, `bin/kiwi-code-<version>.vsix`).
