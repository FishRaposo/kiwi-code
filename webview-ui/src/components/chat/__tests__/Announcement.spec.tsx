		...
		t: (key: string, options?: { version?: string }) => {
			const translations: Record<string, string> = {
				...
				"chat:announcement.release.highlight1":
					"Xiaomi MiMo provider: Added Xiaomi MiMo as a first-class API provider so you can configure MiMo models directly in Kiwi Code.",
				"chat:announcement.release.highlight2":
					"Upstream Kiwi Code handoff: Pulled in the latest upstream sunset merge and related platform updates to keep Kiwi Code aligned with the community handoff work.",
			...
			if (key === "chat:announcement.title" || key === "chat:announcement.finalRelease.title") {
				return `Kiwi Code ${options?.version ?? ""} Released`
			}
...
describe("Announcement", () => {
	it("renders the announcement title and highlights", () => {
		...
		expect(screen.getByText("Kiwi Code 3.55.0 Released")).toBeInTheDocument()
		expect(
			screen.getByText(
				"Xiaomi MiMo provider: Added Xiaomi MiMo as a first-class API provider so you can configure MiMo models directly in Kiwi Code.",
			),
	...
	it("links support users to the Kiwi Code GitHub repository", () => {
		render(<Announcement hideAnnouncement={vi.fn()} />)
