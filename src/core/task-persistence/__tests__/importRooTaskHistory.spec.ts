...
describe("importRooTaskHistory", () => {
	...
	it("resolves Roo and Zoo storage roots from extension domains and configured custom paths", async () => {
		const zooGlobalStoragePath = path.join(tempRoot, "globalStorage", "kiwicodeorganization.kiwi-code")
		...
		expect(result.zooExtensionDomain).toBe("ZooCodeOrganization.kiwi-code")
