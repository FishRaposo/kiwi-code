...
describe("registerCommands handlers", () => {
	...
	it("settingsButtonClicked posts both settingsButtonClicked and didBecomeVisible actions", () => {
		handlers["kiwi-code.settingsButtonClicked"]()
	...
	it("historyButtonClicked posts historyButtonClicked action", () => {
		handlers["kiwi-code.historyButtonClicked"]()
	...
	it("marketplaceButtonClicked posts marketplaceButtonClicked action", () => {
		handlers["kiwi-code.marketplaceButtonClicked"]()
	...
	it("acceptInput posts acceptInput message", () => {
		handlers["kiwi-code.acceptInput"]()
	...
	it("toggleAutoApprove awaits postMessage with toggleAutoApprove action", async () => {
		...
		const handlerPromise = handlers["kiwi-code.toggleAutoApprove"]() as Promise<unknown>
		let settled = false
	...
	it("focusInput awaits postMessage on the registered provider when a sidebar panel is active", async () => {
		...
		const handlerPromise = handlers["kiwi-code.focusInput"]() as Promise<unknown>
		let settled = false
		void handlerPromise.then(() => {
			settled = true
		})
	...
	it("focusInput does not post when no sidebar panel is active", async () => {
		await handlers["kiwi-code.focusInput"]()
	...
	it.each([
		{ command: "kiwi-code.settingsButtonClicked", prefix: "settingsButtonClicked", expectedCalls: 2 },
		{ command: "kiwi-code.historyButtonClicked", prefix: "historyButtonClicked", expectedCalls: 1 },
		{ command: "kiwi-code.marketplaceButtonClicked", prefix: "marketplaceButtonClicked", expectedCalls: 1 },
		{ command: "kiwi-code.acceptInput", prefix: "acceptInput", expectedCalls: 1 },
	])(
	...
	it("toggleAutoApprove logs to outputChannel when postMessageToWebview rejects", async () => {
		...
		await handlers["kiwi-code.toggleAutoApprove"]()
