...
export class Terminal extends BaseTerminal {
	...
	constructor(id: number, terminal: vscode.Terminal | undefined, cwd: string) {
		...
		if (terminal) {
			...
			const options: vscode.TerminalOptions = { cwd, name: "Kiwi Code", iconPath, env }
			...
			if (profileShell?.shellPath) {
				...
				// Kiwi Code's shell-integration controls authoritative.
				if (profileShell.env) {
	 ...
	 * `.vscode/settings.json` to select an executable for Kiwi Code to launch.
	 */
