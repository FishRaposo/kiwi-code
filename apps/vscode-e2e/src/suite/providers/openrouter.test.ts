...
suite("OpenRouter provider", function () {
	...
	test("Should identify as Kiwi Code in outbound DEFAULT_HEADERS", async () => {
		requests.length = 0
		...
		for (const captured of requests) {
			assert.strictEqual(captured.xTitle, "Kiwi Code", "X-Title header should identify the extension as Kiwi Code")
			assert.strictEqual(
				captured.httpReferer,
				"https://github.com/Zoo-Code-Org/Zoo-Code",
				"HTTP-Referer header should point to the Kiwi Code repository",
			)
			assert.ok(
